...
public class TicTacToeClient {
    ...
    public TicTacToeClient(String serverAddress) throws Exception {
        ...
        in = new BufferedReader(new InputStreamReader( //read input written to the socket
            socket.getInputStream()));
        out = new PrintWriter(socket.getOutputStream(), true); //write into the socket
    ...
    public void play() throws Exception {
        ...
        try {
            ...
            if (response.startsWith("WELCOME")) {
                ...
                if (mark == 'X'){
                		icon = new Color(0);
                		opponentIcon = new Color(125);
                }
                else{
                	icon = new Color(125);
            		opponentIcon = new Color(0);
                }
            ...
            while (true) {
                ...
                if (response.startsWith("VALID_MOVE")) {
                    ...
                    currentSquare.setBackground(icon);
                    currentSquare.repaint();
                } else if (response.startsWith("OPPONENT_MOVED")) {
                    int loc = Integer.parseInt(response.substring(15));
                    board[loc].setBackground(opponentIcon);
                    board[loc].repaint();
