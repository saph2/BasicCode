...
background_button_color = "Lavender"
background_tab_color = "PaleGreen"
...
class GUI_Win_Checkbox(GUI_Win):
    ...
    def add_checkbutton(self, option):
        var = StringVar()
        cb = Checkbutton(self.win_frame, text=option, font=selection_font, variable=var, onvalue = option, offvalue = "", command=self.select, background=background_frame_color)
        cb.pack(anchor="w")
...
class GUI_WIN_Tab():
    ...
    def get_selected(self):
        self.selected["scroll"]=self.scroll_win.get_selected()
        self.selected["check"]=self.check_win.get_selected()
        self.selected["files"]=self.files_win.get_selected()
...
class GUI_WIN_Main():
    ...
    def run(self):
        self.selected = self.tabs_win.get_selected()
        for tab in self.selected:
            print (tab)
